...
Now that we have created our example data visualization figure using a query function, we can create other figures using the same process!

For my additional data visualization figure, I will create a scatterplot that will plot the average temperature against time for two user-selected countries! 

This scatterplot can answer the question: **How does the average temperature change over time when comparing two separate countries?** 

<br> 

Using the original query function as a foundation, we can modify our second query function to fit our desired filters. We add two countries as parameters and keep a period range. 
...
```
This query function is fundamentally similar to the original query function except that it accepts two different countries as paramters.
...
Now, to create our scatter plot, we need to create another function that accepts our second query function and uses those parameters to create a figure. 
...
This function is also fundamentally similar to the first geographic scatterplot function we created before. Here, we call the `px.scatter` function instead and we calculate the mean temperature for each year using the `trasform` function! 
...
We see that we have two graphs that show the two separate country scatterplots. Here, we see that there is a clear decreasing trend for Antartica and a very slight increasing trend for Brazil. This function can allow us to compare different countries and how their temperatures are changing over time!
