...
As we can see from the model summary, we can see that the layers for our `title` and `text` components and the `concatenation` portion near the end.
...
As we can see from the figure, each dot represents a word and their weight proportions. The larger the magnitude in either direction, the more times it corresponds to "fake" articles. For instance, the word `trumps` appears on the farthest right spectrum has a large magnitude in the horizontal axis. This suggests that that word appears more in fake news articles. It does seem more related to Trump and it is a strong word with a somewhat negative connotation. 
...
Lastly, a fifth word that we could interpret is `leading`, which appears farthest down from the (0,0). Usually, leading appears in political related news about elections and other relevant topics. Fake news is more prominent in politics, so it makes sense that "leading" is more associated with fake news.

As we can see from these five words, it seems that words more closely related to politics are usually more related to fake news, which logically makes sense since there strong bias and opinions are prominent in that field. 
